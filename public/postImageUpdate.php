...
function ciniki_blog_postImageUpdate(&$ciniki) {
    ...
    $rc = ciniki_blog_checkAccess($ciniki, $args['tnid'], 'ciniki.blog.postImageUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    if( !isset($rc['item']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.blog.29', 'msg'=>'Post image not found'));
    }
    $item = $rc['item'];
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.blog.30', 'msg'=>'You already have an image with this name, please choose another name'));
        }
    }
    ...
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'objectUpdate');
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.blog.postimage', $args['post_image_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    if( isset($args['sequence']) ) {
        ...
        $rc = ciniki_core_sequencesUpdate($ciniki, $args['tnid'], 'ciniki.blog.postimage', 'post_id', $item['post_id'], $args['sequence'], $item['sequence']);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.blog');
            return $rc;
        }
