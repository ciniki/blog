...
// <rsp stat='ok' />
//
function ciniki_blog_postFileUpdate(&$ciniki) {
    //  
	...
	if( !isset($rc['file']) ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1570', 'msg'=>'File does not exist.'));
	}
	$file = $rc['file'];
	...
	if( isset($args['permalink']) ) {
		$strsql = "SELECT id, name, permalink FROM ciniki_blog_post_files "
			...
			. "AND id <> '" . ciniki_core_dbQuote($ciniki, $args['file_id']) . "' "
			. "";
		$rc = ciniki_core_dbHashQuery($ciniki, $strsql, 'ciniki.blog', 'files');
		if( $rc['stat'] != 'ok' ) {
			return $rc;
		}
		if( $rc['num_rows'] > 0 ) {
			return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1571', 'msg'=>'You already have a file with this name, please choose another name.'));
		}
	}
