...
function ciniki_blog_postLinkAdd(&$ciniki) {
    ...
    $rc = ciniki_blog_checkAccess($ciniki, $args['business_id'], 'ciniki.blog.postLinkAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['business_id'], 'ciniki.blog.postlink', $args, 0x07);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $link_id = $rc['id'];
